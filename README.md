...
- **AI-советник (чат)** — раздел «Советник»: диалог с моделью, которая на каждый
  вопрос получает свежие цифры бизнеса (индекс, разрыв, расходы, тренды) и
  отвечает по ним. История — локально в браузере, лимит 40 сообщений/час.
...
### AI-советник и AI-советы (локальная модель через ollama)

На демо-стенде чат работает на локальной модели через [ollama](https://ollama.com):

```bash
ollama serve &                 # если не запущен
ollama pull qwen2.5:3b         # лёгкая быстрая модель (~2 ГБ), отвечает за секунды
# в .env:
...
«Прокачка» модели — одна переменная: поставьте `LLM_MODEL` побольше
(например `qwen2.5:7b`) или укажите `LLM_API_URL` боевого GigaChat/OpenAI.
Думающие модели (deepseek-r1 и т.п.) поддерживаются: размышления вырезаются,
лимит токенов учитывает reasoning.
...
| `LLM_API_URL` / `LLM_API_KEY` / `LLM_MODEL` | — | OpenAI-совместимый LLM для AI-советов и чата-советника (GigaChat/OpenAI/ollama); пусто → только правила, чат отключён |
...
| POST | `/api/chat` | AI-советник: `{"messages":[{role,content}…]}` → `{"reply"}`; контекст бизнеса подставляется на сервере; 503 если LLM не настроен |
